...
- 更新服务启用：正式构建注入 GitHub Releases 更新源（此前未配置，「检查更新」不可用）；现可正常查询远端清单并发现新版本，下载/安装链路沿用既有机 制（SHA-256 校验 + 系统安装页）。

## [1.2.1] - 2026-09-04

### 改进

- 产物库排序改为常驻控件：字段分段（名称/日期/类型）+ 升降序切换，替代弹出菜单；状态常显、无弹层遮挡。
- 排序控件收进「产物库」标题行，筛选 chips 去除类型图标（Material 选中勾保留），头部布局更紧凑。
