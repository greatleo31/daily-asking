# Daily Asking Agent Instructions

始终使用中文回答，保持直接、简洁、工程化，不写空话。

## 项目定位

Daily Asking 是本地优先的 Flutter Android APK，用于记录真实工作经历，通过 BYOK AI 追问补足事实，并生成简历 bullet、周报和面试追问卡。

## 默认工作流

1. 先探索现状，再下结论；能通过代码、配置、OpenSpec 文档确认的，不要靠猜。
2. 非平凡业务变更必须先更新或读取对应 OpenSpec change，再改代码。
3. 优先做最小可验证改动，避免无关重构。
